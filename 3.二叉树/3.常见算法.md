...
- 用 traverse 函数遍历一遍二叉树的所有节点，维护 depth 变量，在叶子节点的时候更新最大深度。
...
- `while` 控制 从上到下 遍历 每一层
- `for` 控制 从左到右 遍历 每一层的每个节点
...
![image-20260119203014096](assets/image-20260119203014096.png)
...
- 再把原来右子树 链接到 root当前的右叶子节点（原来左子树的右叶子节点）。
...
借用辅助集合：先前序遍历得到结果 res集合，然后root依次添加 右节点为 res中的元素
